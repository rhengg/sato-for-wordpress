...
const Register = () => {
  ...
  const canvaCode = params.get("code");
  const canvaState = params.get("state");

  if (!canvaCode || !canvaState) {
    navigator("/not-found");
  }
  ...
  const registerUser = async (e: React.BaseSyntheticEvent) => {
    ...
    const userdata = {
      ...
      otp: pin,
      canva_authorization_code: canvaCode,
      canva_authorization_state: canvaState,
    };
    try {
      ...
      setTimeout(() => {
        if (choosenPlan && choosenPlanAmount) {
